...
ap.add_argument('-flat_restraint_file', default=None, type=str,
                help='File containing flat-bottom restraint parameters')
...
flat_restraint_file = cmd.flat_restraint_file
...
def flat_restraints(system, flat_restraint_file):

	# Creating flat_bond_force
    flat_bond_force = CustomBondForce('step(r-r0)*(k)*(r-r0)^2')
    ...
    for line in open(flat_restraint_file, 'r'):
        if line.strip(): #skip empty lines
            if line[0] != "#" and line[0] != "!" and line[0] != "@":
                segments = line.split()
                atom1 = int(segments[0]) - 1 # convert to 0-index numbering
                atom2 = int(segments[1]) - 1 # convert to 0-index numbering
                r0    = float(segments[2])*angstrom
                k     = float(segments[3])
                if k > 0:
                    flat_bond_force.addBond(atom1, atom2, [r0, k*kilojoule_per_mole/nanometer**2])
...
if flat_restraint_file:
    print("\t- Setting Flat-bottom potential for distances from " + flat_restraint_file + " ...")
    system = flat_restraints(system,flat_restraint_file)
...
for n in range(cmd.firststride, nstride + 1):
    ...
    if os.path.exists(chk_file):
        ...
        if flat_id:
            simulation.reporters.append(ForceReporter(fbout, nsavcrd,forcegroup=flat_id, append=True))
        ...
        simulation.reporters.append(CheckpointReporter(chk_file, nsavcrd))
        if flat_id:
            simulation.reporters.append(ForceReporter(fbout, nsavcrd,forcegroup=flat_id, append=False))
